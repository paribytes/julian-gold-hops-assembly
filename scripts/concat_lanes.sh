...
# Reads raw lanes from the shared (read-only) folder, writes merged output
...
for sample in JG2 JG3; do
    ...
    # Concatenating gzip files directly is valid (gzip streams concatenate cleanly)
    cat $R1_FILES > ${OUT_DIR}/${sample}_R1.fastq.gz
    cat $R2_FILES > ${OUT_DIR}/${sample}_R2.fastq.gz

    # Sanity check: read counts should match between R1 and R2
